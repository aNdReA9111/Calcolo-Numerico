...
#rescale realizza una media dei pixel

#funzioni ausiliarie per operatore di downsampling
...
def A(x, K, sf):    #A = SH
    #applica il blur e poi esegue rescaling
    #serve per costruzione problema set nella super risoluzione
    x = fft.fft2(x)
    y = np.real(fft.ifft2(K * x))
    return rescale(y, 1/sf)

# Moltiplicazione per A trasposta
def AT(x, K, sf):       #AT = (SH)T = HTST
    x = rescale(x, sf)
    x = fft.fft2(x)
